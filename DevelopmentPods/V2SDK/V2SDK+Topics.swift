...
extension V2SDK {
    ...
    public class func getHotTopics(completion: @escaping GenericNetworkingCompletion<TopicList>) {
        let path = "/api/topics/hot.json"
        GenericNetworking.getJSON(path: path, completion: completion)
    }
    
    
    public class func getLatestTopics(completion: @escaping GenericNetworkingCompletion<TopicList>) {
